...
contains
    ...
    subroutine grp_write_fields(Tdomain, parent_id, dim2, out_variables, outputs, ntot_nodes)
        ...
        if (out_variables(OUT_EPS_DEV) == 1) then
            ! EPS_DEV_XX
            call MPI_Gatherv(outputs%eps_dev(0,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! EPS_DEV_YY
            call MPI_Gatherv(outputs%eps_dev(1,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! EPS_DEV_ZZ
            call MPI_Gatherv(outputs%eps_dev(2,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! EPS_DEV_XY
            call MPI_Gatherv(outputs%eps_dev(3,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! EPS_DEV_XZ
            call MPI_Gatherv(outputs%eps_dev(4,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! EPS_DEV_YZ
            call MPI_Gatherv(outputs%eps_dev(5,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            if (Tdomain%nl_flag==1) then
                ! EPS_DEV_PL_XX
                call MPI_Gatherv(outputs%eps_dev_pl(0,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
                ...
                ! EPS_DEV_PL_YY
                call MPI_Gatherv(outputs%eps_dev_pl(1,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
                ...
                ! EPS_DEV_PL_ZZ
                call MPI_Gatherv(outputs%eps_dev_pl(2,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
                ...
                ! EPS_DEV_PL_XY
                call MPI_Gatherv(outputs%eps_dev_pl(3,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
                ...
                ! EPS_DEV_PL_XZ
                call MPI_Gatherv(outputs%eps_dev_pl(4,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
                ...
                ! EPS_DEV_PL_YZ
                call MPI_Gatherv(outputs%eps_dev_pl(5,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                    MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
                if (Tdomain%output_rank==0) then
                    dims(1) = ntot_nodes
        ...
        if (out_variables(OUT_STRESS_DEV) == 1) then
            ...
            ! SIG_DEV_YY
            call MPI_Gatherv(outputs%sig_dev(1,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! SIG_DEV_ZZ
            call MPI_Gatherv(outputs%sig_dev(2,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! SIG_DEV_XY
            call MPI_Gatherv(outputs%sig_dev(3,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! SIG_DEV_XZ
            call MPI_Gatherv(outputs%sig_dev(4,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
            ...
            ! SIG_DEV_YZ
            call MPI_Gatherv(outputs%sig_dev(5,:), dim2, MPI_DOUBLE_PRECISION, all_data_1d, counts, displs, &
                MPI_DOUBLE_PRECISION, 0, Tdomain%comm_output, ierr)
            if (Tdomain%output_rank==0) then
                dims(1) = ntot_nodes
    ...
    subroutine allocate_fields(nnodes, out_flags, fields, nl_flag)
        ...
        if (flag_gradU) then
            allocate(fields%eps_dev(0:5,0:nnodes-1))
            if (nl_flag) then
                allocate(fields%eps_dev_pl(0:5,0:nnodes-1))
            end if
            if (out_flags(OUT_STRESS_DEV) == 1) then
                allocate(fields%sig_dev(0:5,0:nnodes-1))
            end if
        end if
    ...
    subroutine save_field_h5(Tdomain, isort)
            ...
            do k = 0, ngll-1
                do j = 0, ngll-1
                    do i = 0, ngll-1
                        ...
                        if (out_variables(OUT_EPS_DEV   ) == 1) then
                            out_fields%eps_dev(0:5,ind) = eps_dev(i,j,k,0:5)
                            if (allocated(eps_dev_pl)) then
                                out_fields%eps_dev_pl(0:5,ind) = eps_dev_pl(i,j,k,0:5)
                            endif
                        endif
        ...
        do ind = 0,nnodes-1
            if(nData(ind) > 1) then
                ...
                if (out_variables(OUT_EPS_DEV   ) == 1) then
                    ...
                    if (allocated(eps_dev_pl)) then
                        out_fields%eps_dev_pl(0:5,ind) =  out_fields%eps_dev_pl(0:5,ind)/dble(nData(ind))
                    endif
                endif
        ...
        do i = 0,nnodes-1
            if (valence(i)/=0) then
                ...
                if (out_variables(OUT_EPS_VOL   )==1)   out_fields%eps_vol(i)       = out_fields%eps_vol(i)/valence(i)
                if (out_variables(OUT_EPS_DEV   )==1) then
                    out_fields%eps_dev(0:5,i)   = out_fields%eps_dev(0:5,i)/valence(i)
                    if (allocated(out_fields%eps_dev_pl)) then
                        out_fields%eps_dev_pl(0:5,i)   = out_fields%eps_dev_pl(0:5,i)/valence(i)
                    endif
                endif
                if (out_variables(OUT_STRESS_DEV)==1)   out_fields%sig_dev(0:5,i)   = out_fields%sig_dev(0:5,i)/valence(i)
