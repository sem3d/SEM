...
module selement
    ...
    ! Modified by Paul Cupillard 28/02/2006
    ...
    type :: element
       ...
       real, dimension (:,:,:), pointer :: Jacob, Density, Lambda, Kappa, Mu,Q, Qs, Qp, onemSbeta, onemPbeta, MassMat, &
           epsilonvol_, &
           epsilondev_xx_,epsilondev_yy_,epsilondev_xy_,epsilondev_xz_,epsilondev_yz_
       real, dimension (:,:,:,:), pointer :: ACoeff, Forces, Veloc,Displ,Accel, V0, Diagonal_Stress,Residual_Stress, Cij, &
...
contains
    ...
    subroutine Prediction_Elem_Veloc (Elem)
        ...
        !Elem%Forces(1:ngllx-2,1:nglly-2,1:ngllz-2,0:2) = Elem%Displ + dt * Elem%Veloc + dt**2 * (0.5 - bega) * Elem%Accel
        !Elem%V0 = Elem%Veloc
        !Elem%Forces(1:ngllx-2,1:nglly-2,1:ngllz-2,0:2) = alpha * Elem%Forces(1:ngllx-2,1:nglly-2,1:ngllz-2,0:2) + (1-alpha) * Elem%Displ
    ...
    subroutine Correction_Elem_PML_Veloc (Elem, dt)
        ...
        do i = 0,2
            Elem%Veloc1(:,:,:,i) = Elem%DumpVx(:,:,:,0) * Elem%Veloc1(:,:,:,i) + dt * Elem%DumpVx(:,:,:,1)*Elem%Forces1(1:ngllx-2,1:nglly-2,1:ngllz-2,i)
            Elem%Veloc2(:,:,:,i) = Elem%DumpVy(:,:,:,0) * Elem%Veloc2(:,:,:,i) + dt * Elem%DumpVy(:,:,:,1)*Elem%Forces2(1:ngllx-2,1:nglly-2,1:ngllz-2,i)
            Elem%Veloc3(:,:,:,i) = Elem%DumpVz(:,:,:,0) * Elem%Veloc3(:,:,:,i) + dt * Elem%DumpVz(:,:,:,1)*Elem%Forces3(1:ngllx-2,1:nglly-2,1:ngllz-2,i)
        enddo
