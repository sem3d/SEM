        ...
        if (surf%surface_present/=0) then
            ...
            if (Tdomain%nsurfsource(nsurf)%wtype=='A') then
                ...
                if (rg.eq.0) then
                    ...
                    write(*,*) "-------------------------------------"
                    write(*,*) "Va = "// adjustr(Tdomain%nsurfsource(nsurf)%varia)
                    write(*,*) "F1 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcx)
                    if (Tdomain%nsurfsource(nsurf)%dim.gt.1) then
                        write(*,*) "F2 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcy)
                    endif
                    if (Tdomain%nsurfsource(nsurf)%dim.gt.2) then
                        write(*,*) "F3 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcz)
                    endif
                    if (Tdomain%nsurfsource(nsurf)%source.eq."M") then
                        write(*,*) "F4 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcxy)
                        if (surf%surface_dim.gt.2) then
                            write(*,*) "F5 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcxz)
                            write(*,*) "F6 = "// adjustr(Tdomain%nsurfsource(nsurf)%funcyz)
                        endif
                    endif
                    if (Tdomain%nsurfsource(nsurf)%paramvar==1) then
                        write(*,*)
                ...
                if (rg==0) then
                    ...
                    write(*,*) "Load dir : ", Tdomain%nsurfsource(nsurf)%dir(0), &
                        Tdomain%nsurfsource(nsurf)%dir(1), &
                        Tdomain%nsurfsource(nsurf)%dir(2)
                    if (Tdomain%nsurfsource(nsurf)%f0/=0) write(*,*) "Ricker Freqence ", &
                        Tdomain%nsurfsource(nsurf)%f0
                    if (Tdomain%nsurfsource(nsurf)%Rickertau/=0) write(*,*) "Ricker Freqence ", &
                        Tdomain%nsurfsource(nsurf)%Rickertau
                endif
            endif

            if (rg==0) then
                write(*,*) "Source Ref coord   : ", Tdomain%nsurfsource(nsurf)%scoord(0), &
                    Tdomain%nsurfsource(nsurf)%scoord(1), &
                    Tdomain%nsurfsource(nsurf)%scoord(2)
                write(*,*) "Associated surface : ",(Tdomain%nsurfsource(nsurf)%index(i)-1, i=1,Count(Boolean))
                write(*,*) "Load amplitude     : ",  Tdomain%nsurfsource(nsurf)%amplitude
            endif
