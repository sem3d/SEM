           ...
           do n=0,size(Tdomain%sSurfaces)-1
              if (Tdomain%sSurfaces(n)%name == "surface"//adjustl(char(:len_trim(char)))) then
                  ...
                  if (Tdomain%rank == 0) then
                      ...
                      write(*,2009) "Wave Vp : ",  Tdomain%sSurfaces(n)%Elastic%Pspeed
                      write(*,2009) "Wave Vs : ",  Tdomain%sSurfaces(n)%Elastic%Sspeed
                  endif
               endif
           enddo
        enddo
...
!     
!  enddo
!end subroutine renamed_surface
