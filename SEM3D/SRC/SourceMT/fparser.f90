...
MODULE fparser
  ...
  PUBLIC                     :: initf,    & ! Initialize function parser for n functions
                                ...
                                LowCase,  & ! Transform upper case letters in string into lower case     
  ...
  INTEGER(is),                              PARAMETER :: cImmed   = 1,          &
                                                         ...
                                                         cAcos    = 20,         &
                                                         cAtan    = 21,         &
                                                         cHeav    = 22,         & ! added by M Taro
                                                         cfloor   = 23,         & ! added by M Taro
                                                         cDirac   = 24,         & ! added by M Taro
                                                         VarBegin = 25
  ...
  CHARACTER (LEN=5), DIMENSION(cAbs:cDirac), PARAMETER :: Funcs    =(/ 'abs  ', &
                                                                       'exp  ', &
                                                                       'log10', &
                                                                       'log  ', &
                                                                       'sqrt ', &
                                                                       ...
                                                                       'acos ', &
                                                                       'atan ', &
                                                                       'sign ', & ! added by M Taro
                                                                       'floor', & ! added by M Taro
                                                                       'dirac'/)
  TYPE tComp
...
CONTAINS
  ...
  FUNCTION evalf (i, Val) RESULT (res)
    ...
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE  (cHeav); Comp(i)%Stack(SP)=HH(Comp(i)%Stack(SP))
       CASE (cfloor); Comp(i)%Stack(SP)=FLOOR(Comp(i)%Stack(SP))
       CASE (cDirac); Comp(i)%Stack(SP)=DIR(Comp(i)%Stack(SP))
  ...
  SUBROUTINE ParseErrMsg (j, FuncStr, Msg)
    ...
    CHARACTER (len=256)                     :: SourceFile='fparser'
    INTEGER                                 :: k
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    IF (PRESENT(Msg)) THEN
    ...
    END DO
    WRITE(*,'(A)') '?'
    ErrorSMS = 'syntax of function string, '//Msg
  ...
  FUNCTION MathFunctionIndex (str) RESULT (n)
    ...
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    n = 0
    DO j=cAbs,cDirac                                          ! Check all math functions
  ...
  SUBROUTINE Compile (i, F, Var)
    ...
    CHARACTER (LEN=700)                         :: ErrorSMS
    CHARACTER (LEN=256)                         :: SourceFile = 'fparser'
    CHARACTER (LEN=256)                         :: functionName='Compile'
    !----- -------- --------- --------- --------- --------- --------- --------- -------
    ...
    IF (istat /= 0) THEN
       ...
       ErrorSMS = ' Memmory allocation for byte code failed'
       CALL ErrorMessage(ErrorSMS, functionName, SourceFile)
       !STOP
    ELSE
       Comp(i)%ByteCodeSize = 0
...
!   WRITE(*,*)'8. AddCompiledByte ',n
  ...
  !
  FUNCTION HH(xval)   RESULT (res) ! added by M Taro
  ...
  END FUNCTION HH
  !
  FUNCTION DIR(xval)   RESULT (res) ! added by M Taro
