...
subroutine Define_Arrays(Tdomain, rg)
    ...
    integer :: n, mat, ngllx,nglly,ngllz, ngll1,ngll2, ngll,ngllPML,ngllSO, i,j,k, nf,ne,nv,nv_aus, idef, code
    ...
    real, dimension (:,:,:), allocatable :: tempx_PML,tempy_PML
...
!!! Attribute elastic properties from material !!!
    ...
    do n = 0,Tdomain%n_elem-1
        ...
        if (.not. Tdomain%specel(n)%PML) then
            ...
            !   debut introduction des MPML pour probleme de stabilite des PML
            !   (Guillot et Mariotti fevrier 2012)
            allocate (tempx_PML(0:ngllx-1,0:nglly-1,0:ngllz-1))
            allocate (tempy_PML(0:ngllx-1,0:nglly-1,0:ngllz-1))
            if (Tdomain%logicD%MPML ) then
                tempx_PML(:,:,:) = wx(:,:,:)
                tempy_PML(:,:,:) = wy(:,:,:)
                wx(:,:,:) = wx(:,:,:) + Tdomain%MPML_coeff*(wy(:,:,:) + wz(:,:,:))
                wy(:,:,:) = wy(:,:,:) + Tdomain%MPML_coeff*(tempx_PML(:,:,:) + wz(:,:,:))
                wz(:,:,:) = wz(:,:,:) + Tdomain%MPML_coeff*(tempx_PML(:,:,:) + tempy_PML(:,:,:))
            endif
            !   fin introduction des MPML pour probleme de stabilite des PML
            ...
            deallocate (tempx_PML,tempy_PML)
            !  modif mariotti fevrier 2007 cea
            deallocate (Tdomain%specel(n)%InvGrad)  !!initial
