...
subroutine makeCase(nDim, Nmc, corrMod, margiFirst, corrL, &
                        ...
                        folderPath, runPath,&
                        rfPath, statPath, &
                        gen_input_name, mesh_input_name)
        implicit none
        !INPUT
        ...
        character(len=*) :: folderPath
        integer, intent(in) :: localizationLevel
        integer, dimension(:), intent(in) :: nFields
        character(len=*), intent(in) :: rfPath, statPath
        character(len=*), intent(in) :: gen_input_name, mesh_input_name
        !OUTPUT
        ...
        call create_folder(folderPath, ".")


        command_path = string_join_many(folderPath,"/../","run.command")
        gen_path  = string_join_many(folderPath,"/",gen_input_name)
        mesh_path = string_join_many(folderPath,"/",mesh_input_name)
        ...
        jobName = string_join_many("M",numb2String(method),"_")
    ...
    subroutine writeSlurmfile(nDim, nProcsTotal, nProcsPerChunk, nChunks, memPerChunk, wallTime, &
        ...
        integer, intent(in) :: nDim, nProcsTotal, nProcsPerChunk, nChunks, memPerChunk
        character(len=8), intent(in) :: wallTime
        character(len=*), intent(in) :: QManagerFile_path
        character(len=*), intent(in) :: queue
        !character(len=50) :: name
        character(len=*), intent(in) :: jobName
        character(len=*), intent(in) :: rfPath, statPath
    ...
    subroutine create_folder(folder, path)
        ...
        !LOCAL
        character(len = buf_RF) command, fullName
