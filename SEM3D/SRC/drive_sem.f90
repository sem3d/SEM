...
subroutine sem(master_superviseur, communicateur, communicateur_global)
    ...
    !- from elementary to global numbering of GLL points
...
!        sortie_capteur = .FALSE       write(*,*) "itemp:", size(itemp2,1), size(itemp2,2)
...
!    enddo       write(*,*) "itemp:", size(itemp2,1), size(itemp2,2)
!        stop "Incoherent number of elements"
!    end if
!    allocate(Tdomain%specel(0:Tdomain%n_elem-1))
!    do i=0,Tdomain%n_elem-1
...
!        Tdomain%specel(i)%fluid_dirich = itemp2(3,i+1) .ne. 0
!        Tdomain%specel(i)%OUTPUT = .true.
!    enddonit_element(Tdomain%specel(i))
!        Tdomain%specel(i)%mat_index = itemp2(1,i+1)
!        Tdomain%specel(i)%solid = itemp2(2,i+1) .ne. 0
!        Tdomain%specel(i)%fluid_dirich = itemp2(3,i+1) .ne. 0
!        Tdomain%specel(i)%OUTPUT = .true.
...
!    ! n_nodes : number of control nodes
!    deallocate(itemp2)
!    ! Read elements definitions
!    ! n_nodes : number of control nodes
!    deallocate(itemp2)
