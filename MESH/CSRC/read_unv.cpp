...
{
  int inode, exportCoordSysNumb, dispCoordSysNumb, color;

  file >> inode; if ( !file ) return 1; if ( inode == -1 ) return -1; // Stop if block's -1 is reached

  double x = 0., y = 0., z = 0.;
  file >> exportCoordSysNumb >> dispCoordSysNumb >> color >> x >> y >> z;
...
{
  int ielem, etype = 0, phys_prop, mat_prop, color;
  unsigned long long int nbnodes = 0;
  int dummy = 0;
  vector<unsigned long long int> nodeIDs;

  file >> ielem; if ( !file ) return 1; if ( ielem == -1 ) return -1; // Stop if block's -1 is reached

  file >> etype >> phys_prop >> mat_prop >> color >> nbnodes;
  if ( etype < 40 ) file >> dummy >> dummy >> dummy; // 1D element only (rod, beam : type < 40) : additional parameters
  ...
  int dim = 1;
  if ( etype > 40  ) dim = 2; if ( etype > 110 ) dim = 3; // Types are ordered : 1D < 40, 2D >= 40, 3D >= 110
  if ( etype > 130 ) dim = -1;                           // Does no more make sense if > 130
  elems.push_back ( elem { etype, dim, nodeIDs, group { "", -1 } } );
  return 0;
}
...
int read_unv_mesh_groups ( ifstream & file, lsnodes & nodes, lselems & elems, lsgroups & groups )
{
  ...
  file >> dummy; if ( !file ) return 1; if ( dummy == -1 ) return -1; // Stop if block's -1 is reached
...
int read_unv_mesh ( string const & fpath, lsnodes & nodes, lselems & elems, vector<int> * filterelems, vector<string> * filtergroups, bool vtkformat )
{
  ...
  cout << "filterelems  : " << filterelems << endl;
  cout << "filtergroups : " << filtergroups << endl;
  cout << "vtkformat    : " << vtkformat << endl;
