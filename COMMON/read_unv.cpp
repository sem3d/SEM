...
int read_unv_mesh_nodes ( ifstream & file, lsnodes & nodes )
{
    ...
    if ( !file ) return 1;
    if ( dummy == -1 ) return -1; // Stop if block's -1 is reached

    double x = 0., y = 0., z = 0.;
    file >> dummy >> dummy >> dummy >> x >> y >> z;
    if ( !file ) {
        cerr << "read_unv_mesh_nodes : bad node (" << nodes.size () - 1 << ")" << endl;
        return 1;
    }
...
int read_unv_mesh_elems ( ifstream & file, lsnodes & nodes, lselems & elems )
{
    ...
    if ( !file ) {
        cerr << "read_unv_mesh_elems : bad element header" << endl;
        return 1;
    }
    ...
    for ( uint64_t i = 0; i < nbnodes; i++ )
    {
        uint64_t nodeID = 0;
        file >> nodeID;
        nodeID--; // nodeID: 1-based to 0-based
        if ( !file || nodeID >= nodes.size () ) {
            cerr << "read_unv_mesh_elems : bad element (" << elems.size () - 1 << ")" << endl;
            return 1;
        }
    ...
    int dim = 1;
    if ( type > 40  ) dim = 2;
    if ( type > 110 ) dim = 3; // Types are ordered : 1D < 40, 2D >= 40, 3D >= 110
...
int read_unv_mesh_groups ( ifstream & file, lsnodes & nodes, lselems & elems, lsgroups & groups )
{
    ...
    if ( !file ) {
        cerr << "read_unv_mesh_groups : bad group header (" << gpname << ")" << endl;
        return 1;
    }

    group gp { gpname, groups.size () };
    bool added = false;
    for ( uint64_t i = 0; i < nbelems; i++ )
    {
        int type = 0;
        uint64_t tag = 0;
        file >> type >> tag >> dummy >> dummy;
        tag--; // tag: 1-based to 0-based
        if ( type == 7 ) // Group of nodes
        {
            if ( !file || tag >= nodes.size () ) {
                cerr << "read_unv_mesh_groups : bad node group (" << gpname << ", " << tag + 1 << ")" << endl;
                return 1;
            }
            get<3> ( nodes[tag] ) = gp;
            added = true;
        ...
        {
            if ( !file || tag >= elems.size () ) {
                cerr << "read_unv_mesh_groups : bad element group (" << gpname << ", " << tag + 1 << ")" << endl;
                return 1;
            }
            get<3> ( elems[tag] ) = gp;
            added = true;
...
int read_unv_mesh_block ( string const & fpath, int const typeID, lsnodes & nodes, lselems & elems, lsgroups & groups )
{
    ...
    while ( file )
    {
        ...
        if ( minus1 == -1 && type > 0 ) // Read file block by block
        {
            ...
            do
            {
                if      ( type == typeID && typeID == 2411 ) {
                    while ( end == 0 ) {
                        end = read_unv_mesh_nodes  ( file, nodes                );
                    }
                }
                else if ( type == typeID && typeID == 2412 ) {
                    while ( end == 0 ) {
                        end = read_unv_mesh_elems  ( file, nodes, elems         );
                    }
                }
                else if ( type == typeID && typeID == 2467 ) {
                    while ( end == 0 ) {
                        end = read_unv_mesh_groups ( file, nodes, elems, groups );
                    }
                }
...
int filter_unv ( lsnodes & nodes, lselems & elems, vector<int> * filterelems, vector<string> * filtergroups )
{
    ...
    unordered_map<uint64_t, uint64_t> pkmap; // Packed map : non packed / packed ID
    lsnodes pknodes; // Packed nodes
    for ( unsigned int i = 0; i < elems.size (); i++ )
    {
        vector<uint64_t> nodeIDs = get<2> ( elems[i] );
        for ( unsigned int j = 0; j < nodeIDs.size (); j++ )
        {
            uint64_t nodeID = nodeIDs[j];
            if ( pkmap.find ( nodeID ) != pkmap.end () ) nodeIDs[j] = pkmap[nodeID];
            else {
                pknodes.push_back ( nodes[nodeID] );
                nodeIDs[j] = pknodes.size () - 1;
                pkmap[nodeID] = nodeIDs[j];
            }
        }
...
int transform_vtk ( lselems & elems )
{
    ...
    {
        int type = get<0> ( elems[i] );
        vector<uint64_t> nodeIDs, vtkNodeIDs;

        if      ( type == 44 ) return 0; // Quad4 : OK
        else if ( type == 45 )           // Quad8
        {
            nodeIDs = get<2> ( elems[i] );
            for ( unsigned int j = 0; j < nodeIDs.size (); j += 2 )
                vtkNodeIDs.push_back ( nodeIDs[j] ); // Main         nodes
            for ( unsigned int j = 1; j < nodeIDs.size (); j += 2 )
                vtkNodeIDs.push_back ( nodeIDs[j] ); // Intermediate nodes
        }
...
int read_unv_mesh ( string const & fpath, lsnodes & nodes, lselems & elems, vector<int> * filterelems, vector<string> * filtergroups, bool vtkformat )
{
    ...
    for ( unsigned int i = 0; i < groups.size (); i++ )
        cout << "read_unv_mesh : group " << get<0> ( groups[i] ) << " has been found" << endl;
    return 0;
}
