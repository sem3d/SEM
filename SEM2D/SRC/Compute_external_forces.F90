...
subroutine Compute_external_forces (Tdomain,timelocal)
    ...
    do n = 0, Tdomain%n_source-1
        ...
        ! SEM3D Newmark.f90 type-7). Type 3 (fluidpulse) takes f(t) directly as a rate
        ! term into the phi equation -- every acoustic element (iso or aniso) uses the
        ! same velocity-potential formulation, so no per-material special case is needed.
