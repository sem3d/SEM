...
subroutine capteurs_veloc (Tdomain,timelocal,ntime,nelem)
  ...
  if (Tdomain%openfilescapt) then
     ...
     write(51,*) timelocal, Tdomain%specel(nelem)%Strain(0,2,0)
     write(52,*) timelocal, Tdomain%specel(nelem)%Strain(0,2,1)
     write(53,*) timelocal, Tdomain%specel(nelem)%Strain(0,2,2)
     write(54,*) timelocal, Tdomain%specel(nelem)%Veloc(0,2,0)
     write(55,*) timelocal, Tdomain%specel(nelem)%Veloc(0,2,1)
     write(56,*) timelocal, Tdomain%specel(nelem-1)%Strain(ngllx-1,2,0)
     write(57,*) timelocal, Tdomain%specel(nelem-1)%Strain(ngllx-1,2,1)
     write(58,*) timelocal, Tdomain%specel(nelem-1)%Strain(ngllx-1,2,2)
     write(59,*) timelocal, Tdomain%specel(nelem-1)%Veloc(ngllx-1,2,0)
     write(60,*) timelocal, Tdomain%specel(nelem-1)%Veloc(ngllx-1,2,1)
