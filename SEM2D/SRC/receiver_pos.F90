...
subroutine read_receiver_file(Tdomain)
    ...
    ! station_file only feeds sReceiver, i.e. the legacy .vel path (ReceiverPosition +
    ! save_trace, both disabled in main). Traces actually come from the `capteurs` block via
    ! create_capteurs, so an absent station_file is normal and must not abort the run.
