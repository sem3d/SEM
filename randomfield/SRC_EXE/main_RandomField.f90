program main_RandomField
    ...
    !LOCAL VARIABLES
    character(len=buf_RF) :: path, logFilePath, temp_single_path
...
!        outputFolder = "STD_res", &
!        outputName = "Lambida")
!    if(IPT_Temp%rang == 0) call show_IPT_RF(IPT, "IPT_std")
    ...
    call init_IPT_RF(&
        ...
        outputFolder = trim(adjustL(temp_single_path)), &
        outputName = "NewRF")
...
!    if(IPT_Temp%rang == 0) write(*,*) ""
    ...
    contains
        ...
        subroutine init_basic_folders(comm)
            ...
            temp_single_path = string_join_many(results_path,"/",results_folder_name)
            if(IPT_Temp%rang == 0) write(*,*) "     temp_single_path = "//trim(temp_single_path)
